...
define(['creanvas/Core/serverBus', 'modules/Vector', 'creanvas/CollisonSolver/CollisionFactory'], function (serverBus, vector, collisionFactory) {
  
  serverBus.on('applicationCreated', function (appBus) {
    
    var solver = new CollisionSolver();
    
    appBus.on("solveCollisions", function (solidElements) {
      
      solver.solveCollisions(solidElements)
      
      solidElements.forEach(function (solid) {
        solid.element.emit('commitMove', solid.pendingDt);
      });
    });
  });
